...
if check_password():
    ...
    with st.sidebar.expander("🛠 Software Setup (Link ပြောင်းရန်)", expanded=True):
        ...
        if st.button("✅ Link များမှတ်ထားမည်"):
            # မိမိအကောင့်အတွက် သီးသန့် Link သိမ်းဆည်းခြင်း (Refresh လုပ်လည်း မပျောက်ပါ)
    ...
    # --- ၆။ စာရင်းဖျက်ခြင်း (သေချာစီစစ်ပြီးသား အပိုင်း) ---
    if not df.empty:
        st.divider()
        with st.expander("🗑 စာရင်းပြုပြင်ရန်/ဖျက်ရန်", expanded=True):
            # i သည် DataFrame ရဲ့ index ဖြစ်သည်၊ Sheet ထဲမှာ Row နံပါတ်က Header ကြောင့် i + 2 ဖြစ်ရမည်
            for i, row in df.iterrows():
                ...
                if col_y.button("ဖျက်", key=f"del_{i}"):
                    # Apps Script ဘက်က row_index နာမည်နဲ့ လက်ခံအောင် ပို့ပေးရမည်
                    # Sheet ထဲမှာ Index က 0 ကစလို့ မရဘဲ Header ရှိတဲ့အတွက် 2 ကစရမည်
                    delete_payload = {"action": "delete", "row_index": int(i) + 2}
                    try:
                        resp = requests.post(script_url, json=delete_payload)
                        if resp.status_code == 200:
                            st.success(f"{row['Customer']} ကို ဖျက်လိုက်ပါပြီ။")
                            time.sleep(1)
                            st.rerun()
                        else:
                            st.error("❌ ဖျက်လို့မရပါ။ Apps Script ကို ပြန်စစ်ပါ။")
                    except:
