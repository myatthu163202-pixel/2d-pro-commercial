...
# --- ၃။ User Storage (Link တွေ မပျောက်အောင် သေချာမှတ်ထားတာ) ---
...
# Link မရှိရင် ဒီမှာပဲ ရပ်မယ်
if not sheet_url or not script_url:
    st.warning("💡 Setup တွင် Link များကို အရင်ထည့်ပေးပါ။")
    st.stop()

# --- ၆။ Data Loading (ဒီအပိုင်းမှာ Error မတက်အောင် သေချာပြင်ထားတယ်) ---
def get_csv_url(url):
    m = re.search(r"/d/([^/]*)", url)
...
try:
    ...
    # image_65826f.png ပါ error ကို ဒီနေရာမှာ except block နဲ့ သေချာပိတ်ထားတယ်
...
# --- ၇။ Dashboard Layout (image_651592.png UI အတိုင်း) ---
...
# --- ၈။ စာရင်းပြသခြင်းနှင့် တစ်ခုချင်းဖျက်ခြင်း ---
...
if not df.empty:
    for i, row in df.iterrows():
        ...
        # image_6590d9.png မှာပြထားတဲ့ "ဖျက်" ခလုတ် logic
        if col_del.button("🗑 ဖျက်", key=f"del_{i}"):
            # Header Row ပါလို့ +2 လုပ်ရမယ်
            target_row = int(i) + 2
            try:
                # image_6590d9.png ပါ "ဖျက်ပြီးပါပြီ" ဆိုတဲ့ message ပေါ်ဖို့ ဒီမှာ request ပို့တာ
                ...
                # image_658db0.png ပါ "ချိတ်ဆက်မှု Error" ကို ဒီမှာ ကိုင်တွယ်ထားတယ်
