...
# --- ၆။ Data Loading (Cache ပြဿနာ အသေအချာ ရှင်းထားသည်) ---
def get_csv_url(url):
    m = re.search(r"/d/([^/]*)", url)
...
# --- ၈။ ပြင်ဆင်ခြင်းအပိုင်း (တစ်ခုချင်းစီပြင်တာ သေချာပေါက် အလုပ်လုပ်စေရန်) ---
st.divider()
st.subheader("⚙️ စာရင်းများ ပြင်ဆင်ရန်")

if not df.empty:
    for i, row in df.iterrows():
        # Sheet ထဲက Row Index အမှန်ကို တွက်ခြင်း (Header က ၁ ဖြစ်လို့ ၂ ပေါင်းပေးရသည်)
        actual_row = int(i) + 2
        with st.expander(f"👤 {row['Customer']} | 🔢 {row['Number']} | 💰 {row['Amount']} Ks"):
            with st.form(f"edit_form_{i}"):
                ...
                if st.form_submit_button("💾 ပြင်ဆင်မှုသိမ်းမည်"):
                    try:
                        # Google Apps Script ထံသို့ update data ပို့ခြင်း
                        ...
                        if res.status_code == 200:
                            ...
                            # Sheet ဘက်မှာ ဒေတာအမှန်တကယ် ပြောင်းသွားစေရန် ၂ စက္ကန့် စောင့်ခိုင်းခြင်း
                            time.sleep(2) 
                            st.rerun()
                        else:
                            st.error("❌ Apps Script နှင့် ချိတ်ဆက်မှု အဆင်မပြေပါ။")
                    except Exception:
