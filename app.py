...
# --- ၂။ User Database (အကောင့်တစ်ခုနှင့်တစ်ခု ခွဲထားရန်) ---
USERS = {
    "admin": "123456",
    "thiri": "163202"
}
...
with st.sidebar.expander("🛠 Software Setup", expanded=False):
    # သိမ်းထားတဲ့ Link ကို ပြန်ပြပေးခြင်းဖြင့် ထပ်ခါထပ်ခါ ထည့်စရာမလိုတော့ပါ
...
# ပေါက်ဂဏန်းနှင့် ဇ (အဆ) သတ်မှတ်ရန်
st.sidebar.divider()
...
# --- ၆။ Data Loading (Syntax Error များအားလုံးကို ဤနေရာတွင် ပြင်ဆင်ထားသည်) ---
def get_csv_url(url):
    m = re.search(r"/d/([^/]*)", url)
...
try:
    ...
    # image_65826f.png နှင့် image_6514b3.png ပါ 'except' မရှိသော error ကို ဤနေရာတွင် စနစ်တကျ ပြင်ထားသည်
...
# စာရင်းအသစ်သွင်းရန် Form
with st.expander("📝 စာရင်းအသစ်သွင်းရန်"):
    # image_643edc.png ပါ ':' ကျန်ခဲ့သော error ကို ဤနေရာတွင် ပြင်ဆင်ထားသည်
...
with col_del_1:
    st.subheader("🗑 တစ်ခုချင်းဖျက်ရန်")
    if not df.empty:
        for i, row in df.iterrows():
            tx, bt = st.columns([4, 1])
            tx.write(f"👤 {row['Customer']} | 🔢 {row['Number']} | 💵 {int(row['Amount'])} Ks")
            
            # ခလုတ်ကို unique key ပေးထားမှသာ တစ်ခုချင်းစီဖျက်၍ရပါမည်
            if bt.button("ဖျက်", key=f"del_{i}"):
                # Header ကြောင့် index ကို +2 ပေါင်းမှ Google Sheet row အမှန်ရမည်
                target_row = int(i) + 2
                try:
